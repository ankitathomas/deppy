...
func listEntities(ctx context.Context, catsrc *v1alpha1.CatalogSource) ([]*entitysource.Entity, error) {
	conn, err := grpc.ConnectWithTimeout(context.TODO(), catsrc.Address(), 0)
	if conn != nil {
		defer conn.Close()
	}
	if err != nil {
...
func (e BundleEntityID) EntityID() entitysource.EntityID {
	return entitysource.EntityID(fmt.Sprintf("%s/%s", e.Package, e.Version))
}

func entityFromBundle(catsrcName string, bundle *catalogsourceapi.Bundle) (*entitysource.Entity, error) {
	id := idFromBundle(catsrcName, bundle)
...
func idFromBundle(catsrcID string, bundle *catalogsourceapi.Bundle) entitysource.EntityID {
