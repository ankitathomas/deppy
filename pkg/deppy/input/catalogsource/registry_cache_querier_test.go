...
//Iterate GroupBy Filter Get
// StartCache Get Modify Delete StopCache
...
var _ = Describe("Registry EntitySource", func() {
	...
	BeforeEach(func() {
		...
		cli = fake.NewClientBuilder().WithScheme(scheme).WithLists(&v1alpha1.CatalogSourceList{
			...
			//		Spec:       corev1.ServiceSpec{Selector: map[string]string{"olm.catalogSource": "withsvc-catsrc"}},
			//	}},
			//}, &corev1.PodList{
			//	Items: []corev1.Pod{{
			//		ObjectMeta: metav1.ObjectMeta{Labels: map[string]string{"olm.catalogSource": "withsvc-catsrc"}},
			//		Status:     corev1.PodStatus{ContainerStatuses: []corev1.ContainerStatus{{ImageID: "resolved/catsrc/addr"}}},
			//	}},
		}).Build()
		...
		logger := zap.New()
		querier = catalogsource.NewCachedRegistryQuerier(watcher, cli, registryClient, &logger, catalogsource.WithSyncInterval(cacheSyncInterval))
		go func() {
			querier.(*catalogsource.CachedRegistryEntitySource).StartCache(context.TODO())
		}()
	...
	AfterEach(func() {
		querier.(*catalogsource.CachedRegistryEntitySource).StopCache()
	})
	...
	Describe("CachedRegistryEntitySource", func() {
		...
		When("catalogsource changes on cluster", func() {
			...
			FIt("should update entity cache when existing catalogsource changes", func() {
